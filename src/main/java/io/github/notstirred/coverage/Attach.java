...
public class Attach {
    ...
    private static void writeFile(String path, HtmlTag content) {
        Path resolve = USER_DIR.resolve(path);
        try {
            content.with(head(script()
                .withSrc(String.valueOf(resolve.relativize(USER_DIR).resolve("sortable.js")))
            ));

            Files.createDirectories(resolve.getParent());
            Files.write(resolve, content.renderFormatted().getBytes(StandardCharsets.UTF_8));
        } catch (IOException e) {
            System.out.println("Failed to write file: " + resolve.toAbsolutePath());
            e.printStackTrace();
        }
    }
