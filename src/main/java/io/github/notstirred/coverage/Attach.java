...
public class Attach {
    ...
    private static final String SORTABLE_JS = "var stIsIE=!1;\n" +
        ...
        "function darkMode() {\n" +
            ...
            "    style.setProperty(\"--code-color\", \"#181818\");\n" +
            "    style.setProperty(\"--text-color\", \"white\");\n" +
            "    style.setProperty(\"--highlight-color\", \"#373737\");\n" +
            "    style.setProperty(\"--hit-color\", \"#2E7D32\");\n" +
            "    style.setProperty(\"--miss-color\", \"#C62828\");\n" +
            "    style.setProperty(\"--link-color\", \"#64B5F6\");\n" +
            "    \n" +
            "    try {\n" +
            ...
            "function lightMode() {\n" +
            "    let sw = document.getElementById(\"light-dark-mode-switch\");\n" +
            "    sw.textContent = \"Switch to Dark Mode\";\n" +
            "    sw.setAttribute(\"onClick\", \"darkMode()\");\n" +
            "    \n" +
            "    let style = document.querySelector(':root').style;\n" +
            "    style.setProperty(\"--bg-color\", \"#fff\");\n" +
            "    style.setProperty(\"--code-color\", \"#f6f6f6\");\n" +
            "    style.setProperty(\"--text-color\", \"#000\");\n" +
            "    style.setProperty(\"--highlight-color\", \"#ccc\");\n" +
            "    style.setProperty(\"--hit-color\", \"#AADD77\");\n" +
            "    style.setProperty(\"--miss-color\", \"#d99\");\n" +
            "    style.setProperty(\"--link-color\", \"#0D47A1\");\n" +
            "    \n" +
            "    try {\n" +
            ...
            METHOD_INDICES.forEach((packageName, methodsByClass) -> {
                ...
                methodsByClass.forEach((className, methodIndices) -> {
                    ...
                    BodyTag body = body(div(
                            ...
                            div(button("Switch to Dark Mode").withId("light-dark-mode-switch").attr("onClick", "darkMode()")).withStyle("margin-top:auto;margin-bottom:auto;")
                        ...
                        methodData.lines.forEach(line -> {
                            if (line.idx < 0) { // this line isn't tracked by coverage (line numbers, labels, etc.)
                                ...
                                if (data[line.idx] == 1) { // coverage hit
                                    elements.add(addLabelLinks(methodSig, line).withStyle("background-color: var(--hit-color);"));
                                } else { // coverage miss
                                    elements.add(addLabelLinks(methodSig, line).withStyle("background-color: var(--miss-color);"));
                                }
                            }
                        });
                            ...
                            div(
                                details(
                                    summary(methodSig + " " + coverage + "%")
                                        .withStyle("background-color: var(--highlight-color); padding: 10px; border-radius: 2px;"),
                                    pre(elements.toArray(new DomContent[0]))
                                        .withStyle("margin: 0px; padding-bottom: 10px; padding-top: 5px;")
                                ).withCondOpen(true)
                                    .withStyle("background-color: var(--code-color); border-radius: 2px;")
                            ).withStyle("padding: 10px; padding-top: 0px;")
                        ).withStyle("background-color: var(--bg-color); color: var(--text-color);");
                    });
                    ...
                    StyleTag style = style(":root {" +
                            ...
                            "--highlight-color:#ccc;" +
                            "--link-color:#0D47A1;" +
                            "} a { color: var(--link-color); } a:link { color: var(--link-color); } a:visited { color: var(--link-color) };");

                    writeFile(packageName + "/" + className + ".html", html(style, body));
                });
